...
class ManagePage:
    ...
    def merge_tasks(
        ...
        task_keyword_map = {
            str(doc["_id"]): set(doc.get("task", [])) for doc in task_data
        }
        ...
        for idx, task1_id in enumerate(task_nodes):
            ...
            for j, faiss_idx in enumerate(I[idx]):
                ...
                task2_id = next(
                    (key for key, value in task_id_map.items() if value == faiss_idx),
                    None,
                )
        ...
        if paper_type:
            ...
            with col1:
                if st.button("Show Database Content"):
                    documents = list(
                        selected_collection.find({}, {"_id": 1, "content": 1})
                    )
                    if documents:
                        st.session_state["result_log"] = []
            ...
            with col4:
                if st.button("Show Graph Visualization"):
                    ...
                    object_id_tasks = set(
                        map(str, collection_nodes_tasks.distinct("_id"))
                    )
